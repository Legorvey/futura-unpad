...
const ModeratorItem = ({ moderator }: { moderator: ModeratorCardProps }) => {
  ...
  return (
    <div 
      ref={ref} 
      className="relative flex flex-col md:flex-row items-center w-full max-w-4xl gap-8 md:gap-12 group" 
    >
      {/* Static Wrapper */}
      <div 
        className="relative shrink-0 w-full max-w-[320px] aspect-[4/5] md:w-64 md:h-80 md:aspect-auto rounded-[2rem] overflow-hidden shadow-2xl z-0"
      >
      ...
      {/* Typography */}
      <div 
        className="flex flex-col flex-1 z-10 w-full px-4 md:px-0 text-center md:text-left"
      >
        <h4 className="text-lg md:text-lg font-light tracking-tighter text-white/40">
          Moderator
        </h4>
        <h3 className="text-3xl md:text-5xl font-medium tracking-[-0.05em] text-white leading-tight transition-colors duration-500 group-hover:text-amber-300">
          {moderator.name}
        </h3>
        
        {moderator.job && (
          <p className="mt-4 text-base md:text-xl font-light tracking-tight text-white/60">
            {moderator.job}
          </p>
        )}

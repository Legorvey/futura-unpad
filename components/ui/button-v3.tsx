    ...
    <>
      <motion.div whileTap={{ scale: 0.95 }} className={cn("w-fit", className)}>
        <Link 
          ...
          prefetch={true} 
          onClick={handleClick} 
          className="group relative inline-flex h-14 w-fit items-center justify-center overflow-hidden rounded-full bg-white text-black"
        >
          {/* Sliding Background */}
          ...
          {/* Circle with arrow */}
          <div className="absolute left-2 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-black text-white transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:left-[calc(100%-3rem)]">
            <ArrowRight className="h-5 w-5 transition-colors duration-700 group-hover:text-amber-300" />
          </div>
          
          {/* Text */}
          <span className="relative z-10 pl-16 pr-6 text-lg font-medium transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:pl-6 group-hover:pr-16">
            {text}
          </span>
        </Link>
      </motion.div>

...
export default function Navbar() {
    ...
    const [activeDropdown, setActiveDropdown] = useState<"faq" | "registration" | null>(null)
    ...
    const handleHover = (dropdown: "faq" | "registration") => {
        setActiveDropdown(dropdown)
        const ref = dropdown === "faq" ? faqRef : registrationRef
        if (ref.current) {
            setDropdownStyle({
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (window.scrollY > 60 && activeDropdown) {
                // optional: automatically close mega menu on scroll
            ...
            >
                <div
                    className={cn(
                        ...
                        isScrolled
                            ? cn(
                                "w-full max-w-7xl border border-border/70 bg-background/80 backdrop-blur-xl px-4 py-2.5 sm:px-6 shadow-sm",
                                isMobileMenuOpen ? "rounded-3xl" : "rounded-3xl md:rounded-[2rem]"
                              )
                            : "w-full max-w-full rounded-none border border-transparent border-b-border/70 bg-background/95 backdrop-blur-xl px-4 py-3 sm:px-8",
                ...
                >
                    <div className="flex items-center justify-between w-full">
                        ...
                        <nav className="hidden items-center gap-5 text-sm font-medium text-muted-foreground md:flex relative">
                            ...
                            <div 
                                className={cn(
                                    "absolute top-full mt-6 transition-all duration-500 ease-out z-50",
                                    activeDropdown ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                                )}
                                style={{ 
                                    transform: `translateX(calc(${dropdownStyle.left}px - 0.75rem))`,
                                    width: 220,
                                }}
                            >
                                <div className="relative w-full">
                                    ...
                                    >
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <Link href="/registration?program=research-dissemination" className="group flex flex-col gap-1 rounded-2xl p-3 hover:bg-muted transition" onClick={() => setActiveDropdown(null)}>
                                                <span className="text-sm font-semibold text-foreground transition group-hover:text-primary">Research Dissemination</span>
                    ...
                    <div
                        className={cn(
                            "hidden md:block w-full transition-all duration-500 ease-out pointer-events-none",
                            activeDropdown === "registration" ? "h-[240px] opacity-100 border-t border-border/50" : 
                            activeDropdown === "faq" ? "h-[160px] opacity-100 border-t border-border/50" : 
                        ...
                        <nav className="flex flex-col gap-2 pb-4 px-2">
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className={cn("flex flex-col gap-2 overflow-hidden transition-all duration-500 ease-in-out", isMobileRegistrationOpen ? "max-h-48 opacity-100 mt-2" : "max-h-0 opacity-0")}>
                                    ...
                                    <Link href="/registration?program=research-dissemination" className="text-sm font-medium text-muted-foreground hover:text-foreground transition py-2 pl-4 border-l-2 border-border/50 ml-2" onClick={() => setIsMobileMenuOpen(false)}>
